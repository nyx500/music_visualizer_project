function Needles() {
	...
	this.onResize = function() {
		...
		// calculate aspect ratio for radius
		this.dialRadius = (this.plotWidth / this.plotHeight) * this.plotWidth;
		/* Makes sure that the arc-of-ticks never goes outside of the plots,
		which is what was happening before*/
		this.dialRadius = min(this.dialRadius, (this.plotWidth - this.pad) / 2);
		this.dialRadius = min(this.dialRadius, this.plotHeight - this.pad - 10);
	};
	...
	this.needle = function(energy, centreX, bottomY) {
		...
		// subtracted 20 because the needle was overlapping the ticks
		var x = (this.dialRadius - 20) * cos(theta);
		var y = (this.dialRadius - 20) * sin(theta);
		//draw the needle
		line(0, 0, x, y);
		pop();
	};
	...
	this.ticks = function(centreX, bottomY, freqLabel) {
		...
		// Makes ticks a bit thicker because they were not visible on smaller screens
		strokeWeight(2);
